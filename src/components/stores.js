...
function getClasses(year, className , nums, sharp) {
    let res = []
    for (let i = chr2num('A'); i <= chr2num(className); i++) {
        for (let j = 1 ; j <= 2; j++)
            for (let k = 1; k <= 2 ; k++) {
...
let s4 = getClasses(4,'D', 2,false)

export let classes = writable(s1.concat(s2,s3,s4))
